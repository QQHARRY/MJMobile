...
- (void)viewDidLoad
{
    ...
    if(self.navigationController!=nil){
        ...
        _navigationBar.barTintColor = self.navigationController.navigationBar.barTintColor;
        _navigationBar.tintColor = self.navigationController.navigationBar.tintColor;
...
{
//    CLMenuPanel *view = (CLMenuPanel*)_menuView.subviews[0];
//    
//    view.alpha = 0.2;
...
{
    if (_singleEditting && _singleEdittingClass)
    {
        CLMenuPanel*view = nil;
        for (UIView*subView in _menuView.subviews)
        {
            if ([subView isKindOfClass:[CLMenuPanel class]] && ((CLMenuPanel*)subView).toolClass == _singleEdittingClass)
            {
                view = (CLMenuPanel*)subView;
                break;
            }
        }
...
- (void)setMenuView
{
    ...
    if (_singleEditting)
    {
        if (_singleEdittingClass)
        {
            Class superCls = _singleEdittingClass;
            while (superCls)
            {
                superCls = class_getSuperclass(superCls);
                if (superCls == [CLImageToolBase class])
                {
                    [tools addObject:_singleEdittingClass];
                    break;
                }
            }
        }
    ...
    {
        [tools addObjectsFromArray:[CLImageTools list]];
    }
        ...
        else if(image)
        {
            if (_singleEdittingClass)
            {
                if([self.delegate respondsToSelector:@selector(imageEditor:didFinishEdittingWithImage:)]){
                    [self.delegate imageEditor:self didFinishEdittingWithImage:image];
                }
                if (self.navigationController)
                {
                    [self.navigationController popViewControllerAnimated:YES];
