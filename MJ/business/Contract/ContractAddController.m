...
@property (nonatomic, strong) NSArray *ContractDictList; // Contract_task_type
...
- (void)viewDidLoad
{
    ...
    // get dict
    self.ContractDictList = [dictionaryManager getItemArrByType:DIC_APPOINT_EVALUATE];
    ...
    self.typeItem = [RERadioItem itemWithTitle:@"跟进方式" value:@"" selectionHandler:^(RERadioItem *item)
                     {
                         ...
                         for (NSInteger i = 0; i < self.ContractDictList.count; i++)
                         {
                             DicItem *di = [self.ContractDictList objectAtIndex:i];
...
- (RETableViewSection *)addCommitButton
{
    ...
    RETableViewItem *buttonItem = [RETableViewItem itemWithTitle:@"提交" accessoryType:UITableViewCellAccessoryNone selectionHandler:^(RETableViewItem *item)
    {
        ...
        for (DicItem *di in self.ContractDictList)
        {
        ...
        [param setValue:self.contentItem.value forKey:@"task_Contract_content"];
        ...
        SHOWHUD_WINDOW;
        [ContractDataPuller pushNewContractWithParam:param Success:^(NSString *ContractNo)
        {
            HIDEHUD_WINDOW;
